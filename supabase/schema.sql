-- Portfolio schema: run in Supabase SQL Editor (new project).
-- For existing databases missing `featured` on projects, run `supabase/patch_projects_featured.sql` first.
...
create table public.site_profile (
  ...
  tagline text not null default 'Full-stack engineer building reliable products.',
  email text,
  linkedin_url text,
  github_url text,
  resume_url text,
...
create index skills_sort_idx on public.skills (sort_order);
...
create policy "Allow public read site_profile"
...
-- Yaswanth Dokala — full portfolio content (safe to delete this block and insert your own rows)
